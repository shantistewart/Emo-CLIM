...
def get_image_embeddings(model: Any, image_dataset: Dataset, image_dataset_tags: List, device: Any) -> Union[ Tuple[List, List, List], Tuple[List, List] ]:
    ...
    for idx in tqdm.tqdm(range(len(image_dataset)), total=len(image_dataset), desc="Extracting image embeddings and labels"):
        ...
        if tag in image_dataset_tags:
            if model.multi_task:
                ...
                assert tuple(intra_embed.size()) == (model.output_embed_dim, ), "Error with shape of intra_embed."
                assert tuple(cross_embed.size()) == (model.output_embed_dim, ), "Error with shape of cross_embed."
                ...
                assert tuple(embed.size()) == (model.output_embed_dim, ), "Error with shape of embed."
...
def get_audio_embeddings(model: Any, audio_dataset: Dataset, audio_dataset_tags: List, device: Any) -> Union[ Tuple[List, List, List], Tuple[List, List] ]:
    ...
    for idx in tqdm.tqdm(range(len(audio_dataset)), total=len(audio_dataset), desc="Extracting audio embeddings and labels"):
        # get audio and emotion tag:
        audio, tag = audio_dataset[idx]
        # insert batch dimension:
        audio = audio.unsqueeze(dim=0)
        audio = audio.to(device)
        assert len(tuple(audio.size())) == 2, "Error with audio shape."

        # compute audio embedding if emotion tag class was used during training:
        if tag in audio_dataset_tags:
            if model.multi_task:
                with torch.no_grad():
                    intra_embed, cross_embed = model.compute_audio_embeds(audio)
                # remove batch dimension:
                intra_embed = intra_embed.squeeze(dim=0)
                cross_embed = cross_embed.squeeze(dim=0)
                ...
                with torch.no_grad():
                    embed = model.compute_audio_embeds(audio)
                # remove batch dimension:
                embed = embed.squeeze(dim=0)
